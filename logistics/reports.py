...
#                }
    ...
    def on_time_groups(self):
        if self.include_late:
            return [TableData("Non-Reporting", SOHReportingTable(object_list=self.non_reporting,
                                                               request=self._request, 
                                                               prefix='nonreport-',
            month=self.datespan.enddate.month,
            year=self.datespan.enddate.year,
                day=self.datespan.enddate.day)),
             TableData("Late", SOHReportingTable(object_list=self.reported_late,
                                                      request=self._request, 
                                                      prefix='late-',
                 month=self.datespan.enddate.month,
                 year=self.datespan.enddate.year,
                 day=self.datespan.enddate.day)),
             TableData("On-Time", SOHReportingTable(object_list=self.on_time,
                                                         request=self._request, 
                                                         prefix='ontime-',
                 month=self.datespan.enddate.month,
                 year=self.datespan.enddate.year,
            ...
            return [TableData("Non-Reporting", SOHReportingTable(object_list=self.non_reporting,
                ...
                year=self.datespan.enddate.year,
                day=self.datespan.enddate.day)),
                    TableData("Reporting", SOHReportingTable(object_list=self.on_time,
                                                                  request=self._request,
                                                                  prefix='report-',
                        month=self.datespan.enddate.month,
                        year=self.datespan.enddate.year,
